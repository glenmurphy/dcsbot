        ...
        match components.next().as_deref() {
            Some("subscribe") => {
                ...
                if filter.len() > 0 {
                    ...
                    let _ = self.handler_tx.send(HandlerMessage::SubscribeChannel(channel_id, filter_text.to_string()));
                } else {
                    let _ = msg.channel_id.say(&context.http, "Search filter missing. e.g. ```!dcsbot subscribe australia```").await;
                }
            },
            Some("unsubscribe") => {
                let _ = self.handler_tx.send(HandlerMessage::UnsubscribeChannel(channel_id));
            },
            Some(&_) => {},
            None => {
                let _ = msg.channel_id.say(&context.http, "dcsbot commands: ```!dcsbot subscribe <filter>\n!dcsbot unsubscribe```").await;
            },
        }
    }
